...
struct TileMap {
    ...
    QQuick3DTextureData* createTileSpotsTexture(Tile &tile,
                                                QQuick3DObject *parent,
                                                float resolutionScale) {
        if (!tile.spotsPixels)
            return nullptr;
        ...
        const std::vector<uint8_t>& spots = *tile.spotsPixels;
        ...
        for (int y = 0; y < scaledRes; ++y) {
            for (int x = 0; x < scaledRes; ++x) {
                ...
                int byteIndex = origIndex >> 3;
                int bitIndex  = origIndex & 7;
                ptr[y * scaledRes + x] = ((spots[byteIndex] >> bitIndex) & 1) ? 255 : 0;
            }
        }
    ...
    QQuick3DTextureData* getTileSpotsTexture(int tileX, int tileY,
        ...
        if (!tile.spotsTexture || tile.fieldResolutionScale != resolutionScale) {
            tile.spotsTexture = createTileSpotsTexture(tile, parent, resolutionScale);
        }
        return tile.spotsTexture;
    }
